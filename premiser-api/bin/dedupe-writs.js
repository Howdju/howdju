...
function dedupeWrits() {
  return (
    ...
    `
      ...
      .then(({ rows }) => {
        const duplicateWritIdsByMinDuplicateWritId = mapValues(
          groupBy(rows, (row) => row.min_writ_id),
          (group) => map(group, (row) => row.writ_id)
        );
      ...
      .then((duplicateWritIdsByMinDuplicateWritId) => {
        return Promise.all([
          ...
          Promise.all(
            map(
              duplicateWritIdsByMinDuplicateWritId,
              (duplicateWritIds, minDuplicateWritId) =>
                database.query(
                  "dedupeWrits.update",
