  ...
  read(statementCompoundId) {
    const sql = `
        ...
        order by sc.statement_compound_id, sca.order_position
          `
    return query(sql, [statementCompoundId])
        .then( ({rows}) => {
          const row = head(rows)
          if (!row) {
            return null
  ...
  readStatementCompoundEquivalentTo(statementCompound) {
    ...
    return query(sql, args)
        .then( ({rows}) => {
          ...
          if (rows.length > 1) {
            logger.error(`Multiple statement compounds are equivalent to `, statementCompound)
          }
          ...
          forEach(row, (value, name) => {
            ...
            if (match = atomOrderPositionRegExp.exec(name)) {
            ...
            } else if (match = atomStatementTextRegExp.exec(name)) {
              const atomIndex = match[1]
              let atom = atomsByIndex[atomIndex]
              if (!atom) {
                atomsByIndex[atomIndex] = atom = {statementCompoundId}
            ...
            } else if (match = atomStatementIdRegExp.exec(name)) {
              const atomIndex = match[1]
              let atom = atomsByIndex[atomIndex]
              if (!atom) {
                atomsByIndex[atomIndex] = atom = {statementCompoundId}
  ...
  readStatementAtomsByStatementCompoundIdForRootStatementId(rootStatementId) {
    const sql = `
      ...
      order by 
        ...
        sca.order_position
    `
    return query(sql, [rootStatementId, JustificationBasisType.STATEMENT_COMPOUND])
        .then( ({rows}) => {
...
module.exports = new StatementCompoundsDao()