...
const { requireArgs, modelErrorCodes } = require("howdju-common");
...
class SourceExcerptParaphraseValidator {
  ...
  validate(sourceExcerptParaphrase) {
    ...
    const paraphrasingProposition = get(
      sourceExcerptParaphrase,
      "paraphrasingProposition"
    );
    errors.fieldErrors.paraphrasingProposition =
      this.propositionValidator.validate(paraphrasingProposition);
    if (errors.fieldErrors.paraphrasingProposition.hasErrors) {
      errors.hasErrors = true;
    }

    const sourceExcerpt = get(sourceExcerptParaphrase, "sourceExcerpt");
    errors.fieldErrors.sourceExcerpt =
      this.sourceExcerptValidator.validate(sourceExcerpt);
