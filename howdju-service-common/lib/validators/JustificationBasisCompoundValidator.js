...
const { modelErrorCodes, requireArgs } = require("howdju-common");
...
class JustificationBasisCompoundValidator {
  constructor(justificationBasisCompoundAtomValidator) {
    ...
    this.justificationBasisCompoundAtomValidator =
  ...
  validate(justificationBasisCompound) {
    ...
    if (!atoms) {
      ...
      errors.fieldErrors.atoms.itemErrors = map(
        ...
        (atom) => this.justificationBasisCompoundAtomValidator.validate(atom)
      );
      if (
        some(
...
exports.JustificationBasisCompoundValidator =
