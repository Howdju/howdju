...
const { isTruthy, modelErrorCodes } = require("howdju-common");
  ...
  validate(writQuote) {
    ...
    if (has(writQuote, "urls")) {
      if (!isArray(writQuote.urls)) {
        errors.hasErrors = true;
        errors.fieldErrors.urls.modelErrors.push(
          modelErrorCodes.IF_PRESENT_MUST_BE_ARRAY
        );
      } else {
        errors.fieldErrors.urls.itemErrors = map(
          writQuote.urls,
