...
exports.CircularReferenceDetector = class CircularReferenceDetector {
  ...
  detectCircularReferences(toBeStringifiedValue, serializationKeyStack = []) {
    Object.keys(toBeStringifiedValue).forEach((key) => {
      ...
      try {
        ...
        this.logger.debug(
          `path "${joinStrings(
            serializationKeyStack
          )}" JSON.stringify results in error: ${error}`
        );

        let isCircularValue;
        let circularExcludingStringifyResult = "";
        try {
          circularExcludingStringifyResult = JSON.stringify(
            value,
            this.replaceRootStringifyReplacer(value),
            2
          );
        ...
        if (isCircularValue) {
          throw new Error(
            "Circular reference detected:\n" +
              `  Circularly referenced value is value under path "${joinStrings(
                serializationKeyStackWithNewKey
  ...
  replaceRootStringifyReplacer(toBeStringifiedValue) {
    ...
    return function (key, value) {
      if (
        serializedObjectCounter !== 0 &&
        typeof toBeStringifiedValue === "object" &&
        toBeStringifiedValue === value
      ) {
        this.logger.error(
          `object serialization with key ${key} has circular reference to being stringified object`
        );
