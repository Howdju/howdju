...
describe("WritQuotesService", () => {
  describe("createWritQuote", () => {
    it("creates a WritQuote", async () => {
      ...
      const { writQuote: writQuoteOutput } =
        await writQuotesService.createWritQuote({ writQuote: writQuoteInput });
      ...
      expect(writQuotesDao.createWritQuoteUrls).toHaveBeenCalledWith(
        expectedWritQuoteOutput,
        dbUrls,
        userId,
        expect.any(moment)
      );
