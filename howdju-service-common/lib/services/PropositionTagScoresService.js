...
const { JobTypes, JobScopes } = require("../jobEnums");
...
exports.PropositionTagScoresService = class PropositionTagScoresService {
  ...
  setPropositionTagScoresUsingAllVotes() {
    ...
    return this.jobHistoryDao
      .createJobHistory(jobType, JobScopes.FULL, startedAt)
      .then((job) =>
        Promise.all([
          this.propositionTagVotesDao.readVotes(),
          this.propositionTagScoresDao.deleteScoresForType(
            PropositionTagScoreTypes.GLOBAL_VOTE_SUM,
            job.startedAt,
            job.id
          ),
        ])
          ...
          .then(([votes, deletions]) => {
            ...
            this.logger.debug(
              `Recalculating scores based upon ${votes.length} votes`
            );
            return votes;
          })
          .then((votes) =>
            this.processVoteScores(job, votes, this.createPropositionTagScore)
          )
          // TODO(1,2,3): remove exception
          // eslint-disable-next-line promise/no-nesting
          .then((updates) =>
          ...
          .then(() => {
            ...
            return this.jobHistoryDao.updateJobCompleted(
              job,
              JobHistoryStatuses.SUCCESS,
              completedAt
            );
          })
          .catch((err) => {
            const completedAt = utcNow();
            this.jobHistoryDao.updateJobCompleted(
              job,
              JobHistoryStatuses.FAILURE,
              completedAt,
              err.stack
            );
            throw err;
  ...
  updatePropositionTagScoresUsingUnscoredVotes() {
    const startedAt = utcNow();
    this.logger.silly(
      `Starting updatePropositionTagScoresUsingUnscoredVotes at ${startedAt}`
    );
    return this.jobHistoryDao
      .createJobHistory(
        JobTypes.SCORE_PROPOSITION_TAGS_BY_GLOBAL_VOTE_SUM,
        JobScopes.INCREMENTAL,
        startedAt
      )
      .then((job) =>
        this.propositionTagScoresDao
          ...
          .then((votes) => {
            this.logger.debug(
              `Recalculating scores based upon ${votes.length} votes since last run`
            );
            return votes;
          })
          .then((votes) =>
            this.processVoteScores(
              job,
              votes,
              this.createSummedPropositionTagScore
            )
          )
          // TODO(1,2,3): remove exception
          // eslint-disable-next-line promise/no-nesting
          ...
          .then(() => {
            const completedAt = utcNow();
            this.logger.silly(
              `Ending updateJustificationScoresUsingUnscoredVotes at ${completedAt}`
            );
            return this.jobHistoryDao.updateJobCompleted(
              job,
              JobHistoryStatuses.SUCCESS,
  ...
  processVoteScores(job, votes, processVote) {
    return Promise.resolve(
      sumVotesByPropositionIdByTagId(votes, this.logger)
    ).then((voteSumByPropositionIdByTagId) =>
      Promise.all(
        flatMap(
          voteSumByPropositionIdByTagId,
          (voteSumByPropositionId, tagId) =>
            map(voteSumByPropositionId, (voteSum, propositionId) =>
  ...
  createPropositionTagScore(propositionId, tagId, score, job) {
    ...
    return this.propositionTagScoresDao
      ...
      .then((propositionTagScore) => {
        this.logger.debug(
          `Set proposition ${propositionTagScore.propositionId} / tag ${propositionTagScore.tagId}'s score to ${propositionTagScore.score}`
        );
  ...
  createSummedPropositionTagScore(propositionId, tagId, voteSum, job) {
    ...
    return this.propositionTagScoresDao
      ...
      .then((propositionTagScore) => {
        ...
        return this.propositionTagScoresDao.createPropositionTagScore(
          propositionId,
          tagId,
          scoreType,
          newScore,
      ...
      .then((propositionTagScore) => {
        ...
        this.logger.debug(
          `Updated proposition ${propositionTagScore.propositionId} / tag ${propositionTagScore.tagId}'s score to ${propositionTagScore.score} (${diffSign}${voteSum})`
        );
        return propositionTagScore;
      });
  }
...
function sumVotesByPropositionIdByTagId(votes, logger) {
  ...
  forEach(votes, (vote) => {
    ...
    switch (vote.polarity) {
      ...
      default:
        logger.error(
          `Unknown PropositionTagVotePolarities "${vote.polarity} in vote ID ${vote.id}`
        );
        break;
    }
