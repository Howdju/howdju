...
const { rethrowTranslatedErrors } = require("../util");

exports.JustificationsService = class JustificationsService extends (
  EntityService
) {
  constructor(
    config,
  ...
  readJustificationForId(justificationId, { userId }) {
    return this.justificationsDao
      ...
      .then(([justification, rootTarget, targetEntity, basisEntity]) => {
  ...
  readMoreJustifications(continuationToken, count, includeUrls) {
    ...
    return this.justificationsDao
      .readJustifications(filters, sorts, count, true, includeUrls)
      .then((justifications) =>
        validateJustifications(this.logger, justifications)
      )
      .then((justifications) => {
  ...
  async doReadOrCreate(justification, userId, now) {
    if (justification.id) {
      return {
        isExtant: true,
        justification: await this.readJustificationForId(justification.id, {
          userId,
        }),
      };
    }

    return await readOrCreateEquivalentValidJustificationAsUser(
      this,
      justification,
      userId,
      now
  ...
  deleteJustification(authToken, justificationId) {
    return this.authService
      ...
      .then(([userId, now, justification]) =>
        Promise.all([
          userId,
          now,
          justification,
          deleteCounterJustificationsToJustificationIds(
            this.justificationsDao,
            [justification.id],
            userId,
            now
          ),
      ...
      .then(([userId, now, justification, deletedCounterJustificationIds]) =>
        Promise.all([
          userId,
          now,
          this.justificationsDao.deleteJustification(justification, now),
          map(deletedCounterJustificationIds, (id) =>
      ...
      .then(([userId, now, deletedJustificationId]) =>
        Promise.all([
...
function readRootTarget(service, justification, userId) {
  switch (justification.rootTargetType) {
    case JustificationRootTargetTypes.PROPOSITION:
      return service.propositionsService.readPropositionForId(
        justification.rootTarget.id,
        { userId }
      );
    case JustificationRootTargetTypes.STATEMENT:
      return service.statementsService.readStatementForId(
        justification.rootTarget.id,
        { userId }
      );
    default:
      throw newImpossibleError(
        `Exhausted JustificationRootTargetTypess: ${justification.rootTargetType}`
      );
  }
}
...
function readJustificationTarget(service, justificationTarget, { userId }) {
  switch (justificationTarget.type) {
    ...
    case JustificationTargetTypes.JUSTIFICATION:
      return service.readJustificationForId(justificationTarget.entity.id, {
        userId,
      });
...
function readJustificationBasis(service, justificationBasis, { userId }) {
  switch (justificationBasis.type) {
    ...
    case JustificationBasisTypes.PROPOSITION_COMPOUND:
      return service.propositionCompoundsService.readPropositionCompoundForId(
        justificationBasis.entity.id,
        { userId }
      );

    case JustificationBasisTypes.JUSTIFICATION_BASIS_COMPOUND:
      return service.justificationBasisCompoundsService.readJustificationBasisCompoundForId(
        justificationBasis.entity.id,
        { userId }
      );
...
function readOrCreateEquivalentValidJustificationAsUser(
  service,
  justification,
  userId,
  now
  ...
  return Promise.all([
    readOrCreateJustificationTarget(
      service,
      ...
      EntityConflictError,
      UserActionsConflictError,
      rethrowTranslatedErrors("fieldErrors.target")
    ),
    readOrCreateJustificationBasis(
      service,
      justification.basis,
      userId,
      now
      ...
      EntityConflictError,
      UserActionsConflictError,
      rethrowTranslatedErrors("fieldErrors.basis")
    ),
    now,
  ])
    .then(([{ targetEntity }, { basisEntity }, now]) => {
      ...
      switch (justification.target.type) {
        ...
        case JustificationTargetTypes.JUSTIFICATION:
          ...
          justification.rootTargetType =
            justification.target.entity.rootTargetType;
          break;
        default:
    ...
    .then(([now, justification, equivalentJustification]) =>
      Promise.all([
        now,
        justification,
        !!equivalentJustification,
        equivalentJustification ||
          service.justificationsDao.createJustification(
            justification,
            userId,
            now
          ),
...
function readOrCreateJustificationTarget(
  service,
  justificationTarget,
  userId,
  now
  ...
  switch (justificationTarget.type) {
    case JustificationTargetTypes.PROPOSITION:
      return service.propositionsService
        .readOrCreateValidPropositionAsUser(
        ...
        .catch(
          ...
          EntityConflictError,
          UserActionsConflictError,
          rethrowTranslatedErrors("fieldErrors.entity")
        )
    ...
    case JustificationTargetTypes.STATEMENT:
      return Promise.resolve(
        service.statementsService.doReadOrCreate(
          justificationTarget.entity,
          userId,
          now
        )
    ...
    case JustificationTargetTypes.JUSTIFICATION:
      return Promise.resolve(
        service.doReadOrCreate(justificationTarget.entity, userId, now)
      )
        .catch(
          EntityValidationError,
          EntityConflictError,
...
function readOrCreateJustificationBasis(
  service,
  justificationBasis,
  userId,
  now
  ...
  switch (justificationBasis.type) {
    case JustificationBasisTypes.WRIT_QUOTE:
      return service.writQuotesService
        .readOrCreateWritQuoteAsUser(justificationBasis.entity, userId, now)
        .catch(
          EntityValidationError,
          EntityConflictError,
          UserActionsConflictError,
    ...
    case JustificationBasisTypes.PROPOSITION_COMPOUND:
      ...
      return service.propositionCompoundsService
        .createPropositionCompoundAsUser(justificationBasis.entity, userId, now)
        .catch(
          EntityValidationError,
          EntityConflictError,
          UserActionsConflictError,
    ...
    case JustificationBasisTypes.JUSTIFICATION_BASIS_COMPOUND:
      return service.justificationBasisCompoundsService
    ...
    default:
      throw newImpossibleError(
        `Unsupported JustificationBasisTypes: ${justificationBasis.type}`
      );
  }
}
    ...
    return (
      justificationsDao
        ...
        .then((currentlyDeletedJustificationIds) =>
          deleteCounterJustificationsToJustificationIds(
            justificationsDao,
            currentlyDeletedJustificationIds,
            userId,
            now,
