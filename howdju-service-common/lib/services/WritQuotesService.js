...
const { WritQuoteValidator } = require("../validators");
const {
...
exports.WritQuotesService = class WritQuotesService {
  ...
  readInitialWritQuotes(requestedSorts, count) {
    ...
    return this.writQuotesDao
      .readWritQuotes(unambiguousSorts, count)
      .then((writQuotes) => {
  ...
  readMoreWritQuotes(continuationToken, count) {
    ...
    return this.writQuotesDao
      .readMoreWritQuotes(sorts, count)
      .then((writQuotes) => {
        const nextContinuationToken =
          createNextContinuationToken(sorts, writQuotes, filters) ||
          continuationToken;
        return {
          writQuotes,
  ...
  async createWritQuote({ authToken, writQuote }) {
    ...
    let writ;
    const equivalentWrit = (writ = await this.writsDao.readWritEquivalentTo(
      writQuote.writ
    ));
    if (equivalentWrit) {
      writQuote.writ = equivalentWrit;

      const equivalentWritQuote =
        await this.writQuotesDao.readWritQuoteEquivalentTo(writQuote);
      if (equivalentWritQuote) {
        equivalentWritQuote.writ = equivalentWrit;

        // Creating an equivalent writQuote will only add URLs.
        const { addedUrls } = diffUrls(
          equivalentWritQuote.urls,
          writQuote.urls
        );
        if (addedUrls.length) {
      ...
      writQuote.writ = writ = await this.writsDao.createWrit(
        writQuote.writ,
        userId,
        now
      );
    ...
    writQuote = await this.writQuotesDao.createWritQuote(
      writQuote,
      userId,
      now
    );
    ...
    writQuote.urls = await this.urlsService.readOrCreateUrlsAsUser(
      urls,
      userId,
      now
    );
    ...
    return this.authService
      ...
      .then(([userId, writQuoteHasChanged, writHasChanged, urlDiffs]) => {
        ...
        if (writQuoteHasChanged) {
          ...
          assign(userActionConflicts, {
            [userActionsConflictCodes.OTHER_USERS_HAVE_VOTED_ON_JUSTIFICATIONS_BASED_ON_THIS_WRIT_QUOTE]:
              this.writQuotesDao.isBasisToJustificationsHavingOtherUsersVotes(
                userId,
                writQuote
              ),
            [userActionsConflictCodes.OTHER_USERS_HAVE_BASED_JUSTIFICATIONS_ON_THIS_WRIT_QUOTE]:
              this.writQuotesDao.isBasisToOtherUsersJustifications(
                userId,
                writQuote
              ),
            [userActionsConflictCodes.OTHER_USERS_HAVE_COUNTERED_JUSTIFICATIONS_BASED_ON_THIS_WRIT_QUOTE]:
              this.writQuotesDao.isBasisToJustificationsHavingOtherUsersCounters(
                userId,
                writQuote
              ),
          });
        ...
        if (writHasChanged) {
          ...
          assign(userActionConflicts, {
            [userActionsConflictCodes.OTHER_USERS_HAVE_VOTED_ON_JUSTIFICATIONS_BASED_ON_THIS_WRIT]:
              this.writsDao.isWritOfBasisToOtherUsersJustifications(
                userId,
                writ
              ),
            [userActionsConflictCodes.OTHER_USERS_HAVE_BASED_JUSTIFICATIONS_ON_THIS_WRIT]:
              this.writsDao.isWritOfBasisToJustificationsHavingOtherUsersVotes(
                userId,
                writ
              ),
            [userActionsConflictCodes.OTHER_USERS_HAVE_COUNTERED_JUSTIFICATIONS_BASED_ON_THIS_WRIT]:
              this.writsDao.isWritOfBasisToJustificationsHavingOtherUsersCounters(
                userId,
                writ
              ),
          });
        ...
        return Promise.props({
          ...
          writHasChanged,
          urlDiffs,
          hasPermission: this.permissionsDao.userHasPermission(
            userId,
        ...
        ({
          ...
          if (entityConflictCodes.length > 0) {
            throw new EntityConflictError(
              merge(WritQuoteValidator.blankErrors(), {
                hasErrors: true,
          ...
          if (userActionsConflictCodes.length > 0) {
            ...
            this.logger.info(
              `User ${userId} overriding userActionsConflictCodes ${userActionsConflictCodes}`
            );
          }
      ...
      .then(([userId, now, writQuote, writ, urls, writQuoteHasChanged]) => {
        if (writQuoteHasChanged) {
          this.actionsService.asyncRecordAction(
            userId,
            now,
  ...
  updateWritQuoteUrlsAsUser(writQuote, urlDiffs, userId, now) {
    return Promise.resolve()
      ...
      .then((dedupedUrls) =>
        groupBy(dedupedUrls, (u) => (u.id ? "hasId" : "noId"))
      )
      .then(({ hasId: extantUrls, noId: newUrls }) =>
        Promise.all([
          extantUrls || [],
          newUrls
            ? this.urlsService.readOrCreateUrlsAsUser(newUrls, userId, now)
            : [],
        ])
      )
      ...
      .then(({ updatedUrls, addedUrls, removedUrls }) =>
        Promise.all([
          updatedUrls,
          addedUrls.length > 0
            ? this.writQuotesDao.createWritQuoteUrls(
                writQuote,
                addedUrls,
                userId,
                now
          ...
          removedUrls.length > 0
            ? this.writQuotesDao.deleteWritQuoteUrls(
                writQuote,
                removedUrls,
                now
              )
      ...
      .then(([updatedUrls, createdWritQuoteUrls, deletedWritQuoteUrls]) => {
        map(createdWritQuoteUrls, (writQuoteUrl) =>
          this.actionsService.asyncRecordAction(
            ...
            ActionTargetTypes.WRIT_QUOTE,
            writQuoteUrl.writQuoteId,
            ActionSubjectTypes.URL,
            writQuoteUrl.urlId
          )
        );
  ...
  readOrCreateValidWritQuoteAsUser(writQuote, userId, now) {
    return Promise.resolve().then(() => {
      ...
      return readOrCreateEquivalentValidWritQuoteAsUser(
        this,
        writQuote,
        userId,
        now
...
function readOrCreateEquivalentValidWritQuoteAsUser(
  service,
  writQuote,
  userId,
  now
  ...
  return Promise.all([
    service.writsService.readOrCreateValidWritAsUser(
      writQuote.writ,
      userId,
      now
    ),
    ...
    .then(([writ, urls, { isExtant, writQuote }, oldUrls]) => {
      ...
      map(zip(oldUrls, urls), ([oldUrl, url]) => {
        assign(oldUrl, url);
      });
...
function readOrCreateJustWritQuoteAsUser(service, writQuote, userId, now) {
  return service.writQuotesDao
    .readWritQuoteHavingWritIdAndQuoteText(
      writQuote.writ.id,
      writQuote.quoteText
    )
    .then((equivalentWritQuote) =>
      Promise.all([
        !!equivalentWritQuote,
        equivalentWritQuote ||
          service.writQuotesDao.createWritQuote(writQuote, userId, now),
      ])
    )
  ...
  return service.writQuotesDao
    ...
    .then((extantWritQuoteUrls) => {
      ...
      return map(writQuote.urls, (url) => {
        ...
        return (
          Promise.all([
            service.writQuotesDao.createWritQuoteUrl(
              writQuote,
              url,
              userId,
              now
            ...
            url.target &&
              service.writQuotesDao.createWritQuoteUrlTarget(
                writQuote,
                url,
                userId,
                now
