...
exports.PropositionCompoundsService = class PropositionCompoundsService {
  ...
  constructor(propositionCompoundValidator, actionsService, propositionsService, propositionCompoundsDao) {
  ...
  createPropositionCompoundAsUser(propositionCompound, userId, now) {
    return Promise.resolve()
      .then(() => {
        const validationErrors = this.propositionCompoundValidator.validate(propositionCompound)
        if (validationErrors.hasErrors) {
          throw new EntityValidationError(validationErrors)
        }
        return userId
      })
      .then(userId => this.createValidPropositionCompoundAsUser(propositionCompound, userId, now))
  }

  createValidPropositionCompoundAsUser(propositionCompound, userId, now) {
    return Promise.resolve()
      .then(() => propositionCompound.id ?
        propositionCompound :
        this.propositionCompoundsDao.readPropositionCompoundEquivalentTo(propositionCompound)
      )
      .then(equivalentPropositionCompound => {
        const isExtant = !!equivalentPropositionCompound
        return Promise.all([
          isExtant,
          isExtant ? equivalentPropositionCompound : this.propositionCompoundsDao.createPropositionCompound(userId, propositionCompound, now),
  ...
  createPropositionCompoundAtoms(propositionCompound, propositionCompoundAtoms, userId, now) {
    return Promise.resolve()
      .then(() => Promise.all(map(propositionCompoundAtoms, atom =>
        atom.entity.id ?
          [atom, {isExtant: true, proposition: atom.entity}] :
          Promise.all([atom, this.propositionsService.readOrCreatePropositionAsUser(atom.entity, userId, now)])
      )))
      .then(atomsWithPropositions => map(atomsWithPropositions, ([atom, {proposition}]) => {
