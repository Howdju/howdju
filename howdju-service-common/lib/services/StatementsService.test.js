...
describe('StatementsService', () => {
  describe('doReadOrCreate', () => {
    test('creates a statement', async () => {
      ...
      sinon.assert.callCount(propositionsService.readOrCreateValidPropositionAsUser, 1)
      sinon.assert.calledWith(propositionsService.readOrCreateValidPropositionAsUser, proposition, userId, now)
      sinon.assert.callCount(persorgsService.readOrCreateValidPersorgAsUser, 2)
      sinon.assert.calledWith(persorgsService.readOrCreateValidPersorgAsUser.getCall(0), speaker1, userId, now)
      sinon.assert.calledWith(persorgsService.readOrCreateValidPersorgAsUser.getCall(1), speaker2, userId, now)
      sinon.assert.callCount(statementsDao.createStatement, 2)
      sinon.assert.calledWith(statementsDao.createStatement.getCall(0), innerStatement, userId, now)
      sinon.assert.calledWith(statementsDao.createStatement.getCall(1), statement, userId, now)
    })
  })
})
