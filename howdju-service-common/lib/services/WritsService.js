...
const { RequestValidationError } = require("../serviceErrors");
...
exports.WritsService = class WritsService {
  ...
  readInitialWrits(requestedSorts, count) {
    ...
    return this.writsDao.readWrits(unambiguousSorts, count).then((writs) => {
  ...
  readMoreWrits(continuationToken, count) {
    ...
    return this.writsDao.readMoreWrits(sorts, count).then((writs) => {
      const nextContinuationToken =
        createNextContinuationToken(sorts, writs, filters) || continuationToken;
      return {
        writs,
  ...
  updateWritAsUser(userId, writ, now) {
    return this.writsDao.update(writ).then((writ) => {
      this.actionsService.asyncRecordAction(
        userId,
        now,
        ActionTypes.UPDATE,
        ActionTargetTypes.WRIT,
        writ.id
      );
      return writ;
    });
