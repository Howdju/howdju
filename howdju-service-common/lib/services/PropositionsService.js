...
const { PropositionValidator } = require("../validators");
...
exports.PropositionsService = class PropositionsService {
    ...
    return Promise.resolve()
      ...
      .then(([proposition, tags, recommendedTags, propositionTagVotes]) => {
        ...
        if (propositionTagVotes) {
          ...
          proposition.propositionTagVotes = filter(
            propositionTagVotes,
            (vote) => some(proposition.tags, (tag) => tagEqual(tag, vote.tag))
          );
        }
        return proposition;
      });
  ...
  readInitialPropositions(requestedSorts, count) {
    ...
    return this.propositionsDao
      .readPropositions(unambiguousSorts, count)
      .then((propositions) => {
  ...
  readMorePropositions(continuationToken, count) {
    ...
    return this.propositionsDao
      .readMorePropositions(sorts, count)
      .then((propositions) => {
  ...
  deleteProposition(authToken, propositionId) {
    return this.authService
      ...
      .then(([userId, now, proposition, dependentJustifications]) =>
        Promise.all([
          userId,
          now,
          this.propositionsDao.deleteProposition(proposition, now),
          this.justificationsDao.deleteJustifications(
            dependentJustifications,
            now
          ),
        ])
  ...
  readOrCreateProposition(authToken, proposition) {
    return this.authService.readUserIdForAuthToken(authToken).then((userId) => {
      const now = new Date();
  ...
  readOrCreatePropositionAsUser(proposition, userId, now) {
    return Promise.resolve()
      ...
      .then((userId) =>
        this.readOrCreateValidPropositionAsUser(proposition, userId, now)
      );
  }
  ...
  readOrCreateValidPropositionAsUser(proposition, userId, now) {
    return Promise.resolve()
      .then(() => {
        ...
        return readOrCreateEquivalentValidPropositionAsUser(
          this,
          proposition,
          userId,
          now
      ...
      .then((wrapper) => {
        if (proposition.tags) {
          ...
          return readOrCreateTagsAndVotes(
            this,
            userId,
            wrapper.proposition.id,
            proposition.tags,
            now
  ...
  readPropositionsForTagId(tagId, { userId = undefined, authToken }) {
    return Promise.resolve()
      ...
      .then((userId) =>
        Promise.all([
          this.propositionTagsService.readPropositionsRecommendedForTagId(
            tagId
          ),
          userId
      ...
      .then(
        ([recommendedPropositions, userTaggedPropositionsByVotePolarity]) => {
          const {
...
function readOrCreateEquivalentValidPropositionAsUser(
  service,
  proposition,
  userId,
  now
  ...
  return Promise.resolve()
    ...
    .then(([userId, extantProposition, now]) => {
      ...
      return Promise.all([
        ...
        isExtant
          ? extantProposition
          : service.propositionsDao.createProposition(userId, proposition, now),
      ]);
    })
      ...
      map(tags, (tag) =>
        service.tagsService
          ...
          .then((tag) => {
            ...
            return Promise.all([
              tag,
              service.propositionTagVotesService.readOrCreatePropositionTagVoteAsUser(
                userId,
                propositionTagVote,
                now
              ),
