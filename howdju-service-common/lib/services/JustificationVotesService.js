...
const { EntityTypes } = require("howdju-common");
...
exports.JustificationVotesService = class JustificationVotesService {
    ...
    return this.authService
      ...
      .then(([userId, updatedOpposingVoteIds, equivalentVotes]) => {
        if (updatedOpposingVoteIds.length > 0) {
          this.logger.debug(
            `Deleted ${updatedOpposingVoteIds.length} opposing justification votes`,
            { vote }
          );
        }
        if (equivalentVotes.length > 0) {
          if (equivalentVotes.length > 1) {
            this.logger.error(
              `${equivalentVotes.length} equivalent justification votes exist`,
              { equivalentVotes, vote }
            );
          }
          const equivalentVote = equivalentVotes[0];
          this.logger.debug("Equivalent vote already exists", {
            equivalentVote,
            vote,
          });
  ...
  deleteVote(authToken, vote) {
    return this.authService
      ...
      .then((userId) =>
        this.justificationVotesDao.deleteEquivalentVotes(userId, vote)
      )
      .then((deletedVoteIds) => {
