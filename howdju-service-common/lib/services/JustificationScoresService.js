...
const sumVotesByJustificationId = (votes, logger) => {
  ...
  forEach(votes, (vote) => {
    ...
    switch (vote.polarity) {
      ...
      default:
        logger.error(
          `Unknown JustificationVotePolarities "${vote.polarity} in vote ID ${vote.id}`
        );
        break;
    }
...
exports.JustificationScoresService = class JustificationScoresService {
    ...
    return this.jobHistoryDao
      ...
      .then((job) =>
        Promise.all([
          ...
          this.justificationScoresDao.deleteScoresForType(
            JustificationScoreTypes.GLOBAL_VOTE_SUM,
            job.startedAt,
            job.id
          ),
        ])
          ...
          .then((votes) =>
            this.processVoteScores(job, votes, this.createJustificationScore)
          )
          // eslint-disable-next-line promise/no-nesting
          .then((updates) =>
  ...
  updateJustificationScoresUsingUnscoredVotes() {
    const startedAt = utcNow();
    this.logger.silly(
      `Starting updateJustificationScoresUsingUnscoredVotes at ${startedAt}`
    );
    return this.jobHistoryDao
      .createJobHistory(
        JobTypes.SCORE_JUSTIFICATIONS_BY_GLOBAL_VOTE_SUM,
        JobScopes.INCREMENTAL,
        startedAt
      )

      .then((job) =>
        this.justificationScoresDao
          ...
          .then((votes) =>
            this.processVoteScores(
              job,
              votes,
              this.createSummedJustificationScore
            )
          )
          // TODO(1,2,3): remove exception
          // eslint-disable-next-line promise/no-nesting
  ...
  processVoteScores(job, votes, processVote) {
    return Promise.resolve(sumVotesByJustificationId(votes, this.logger)).then(
      (voteSumByJustificationId) =>
        Promise.all(
          map(voteSumByJustificationId, (voteSum, justificationId) =>
  ...
  createJustificationScore(justificationId, score, job) {
    ...
    return this.justificationScoresDao
      ...
      .then((justificationScore) => {
        this.logger.debug(
          `Set justification ${justificationScore.justificationId}'s score to ${justificationScore.score}`
        );
  ...
  createSummedJustificationScore(justificationId, voteSum, job) {
    ...
    return this.justificationScoresDao
      ...
      .then((justificationScore) => {
        const currentScore = justificationScore ? justificationScore.score : 0;
        const newScore = currentScore + voteSum;
        return this.justificationScoresDao.createJustificationScore(
          justificationId,
          scoreType,
          newScore,
          job.startedAt,
      ...
      .then((justificationScore) => {
        ...
        this.logger.debug(
          `Updated justification ${justificationScore.justificationId}'s score to ${justificationScore.score} (${diffSign}${voteSum})`
        );
        return justificationScore;
      });
  }
