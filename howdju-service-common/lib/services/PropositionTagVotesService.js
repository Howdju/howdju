...
exports.PropositionTagVotesService = class PropositionTagVotesService {
  ...
  readVotesForPropositionIdAsUser(userId, propositionId) {
    return this.propositionTagVotesDao.readVotesForPropositionIdAsUser(
      userId,
      propositionId
    );
  }
  ...
  readOrCreatePropositionTagVote(authToken, propositionTagVote) {
    return this.authService
      ...
      .then((userId) =>
        this.readOrCreatePropositionTagVoteAsUser(
          userId,
          propositionTagVote,
          new Date()
        )
  ...
  readOrCreatePropositionTagVoteAsUser(userId, propositionTagVote, now) {
    return Promise.resolve()
      ...
      .then((userId) =>
        this.readOrCreateValidPropositionTagVoteAsUser(
          userId,
          propositionTagVote,
          now
        )
    ...
    return Promise.resolve()
      .then(() => {
        if (propositionTagVote.id) {
          return this.propositionTagVotesDao.readPropositionTagVoteForId(
            propositionTagVote.id
          );
        }
        return null;
      })
      .then((extantVote) => {
        ...
        return (
          this.tagsService
            .readOrCreateValidTagAsUser(userId, propositionTagVote.tag, now)
            // TODO(1,2,3): remove exception
            // eslint-disable-next-line promise/no-nesting
            .then((tag) =>
              Promise.all([
                this.propositionTagVotesDao.readPropositionTagVote(
                  userId,
      ...
      .then(({ redundantVote, tag }) => {
        ...
        // TODO(1,2,3): remove exception
        // eslint-disable-next-line promise/no-nesting
        return this.propositionTagVotesDao
