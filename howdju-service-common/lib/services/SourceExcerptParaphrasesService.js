    ...
    return Promise.resolve().then(() => {
      if (sourceExcerptParaphrase.id) {
        return Promise.props({
          isExtant: true,
          sourceExcerptParaphrase: this.readSourceExcerptParaphraseForId(
            sourceExcerptParaphrase.id
          ),
        });
      }

      return readOrCreateEquivalentValidSourceExcerptParaphraseAsUser(
        this,
        sourceExcerptParaphrase,
        userId,
        now
...
function getSourceExcerptEntity(
  ...
  switch (sourceExcerptType) {
    ...
    case SourceExcerptTypes.VID_SEGMENT:
      return service.vidSegmentsService.readVidSegmentForId(
        sourceExcerptEntityId,
        { userId }
      );
    default:
      throw newImpossibleError(
        `Impossible SourceExcerptTypes: ${sourceExcerptType}`
      );
  }
}
...
function readOrCreateEquivalentValidSourceExcerptParaphraseAsUser(
  ...
  return Promise.all([
    ...
    readOrCreateSourceExcerptEntity(
      service,
      sourceExcerptType,
      sourceExcerptEntity,
      userId,
      ...
      ([
        ...
        if (isPropositionExtant && isSourceExcerptExtant) {
          service.logger.debug(
            `Found extant proposition (ID ${proposition.id}) and sourceExcerpt (ID ${sourceExcerptEntity.id}  Attempting to find extant sourceExcerptParaphrase from them.`
          );
          const sourceExcerptType = sourceExcerptParaphrase.sourceExcerpt.type;
          ...
          return service.sourceExcerptParaphrasesDao
            .readSourceExcerptHavingPropositionIdAndSourceExcerptTypeAndId(
              proposition.id,
              sourceExcerptType,
              sourceExcerptEntity.id
            )
            .then((extantSourceExcerptParaphrase) => {
              if (extantSourceExcerptParaphrase) {
                service.logger.debug(
                  `Found extant sourceExcerptParaphrase (ID: ${extantSourceExcerptParaphrase.id} based upon proposition and sourceExcerpt`
                );
                extantSourceExcerptParaphrase.paraphrasingProposition =
                  proposition;
                extantSourceExcerptParaphrase.sourceExcerpt.entity =
                  sourceExcerptEntity;
                return {
                  isExtant: true,
              ...
              service.logger.debug(
                `Did not find extant sourceExcerptParaphrase based upon proposition and sourceExcerpt`
              );
              sourceExcerptParaphrase.paraphrasingProposition = proposition;
              sourceExcerptParaphrase.sourceExcerpt.entity =
                sourceExcerptEntity;
              return (
                service.sourceExcerptParaphrasesDao
                  ...
                  .then((sourceExcerptParaphrase) => {
                    service.logger.debug(
                      `Created sourceExcerptParaphrase (ID ${sourceExcerptParaphrase.id})`
                    );
                    return {
                      isExtant: false,
                      sourceExcerptParaphrase,
    ...
    .then(({ isExtant, sourceExcerptParaphrase }) => {
      ...
      service.actionsService.asyncRecordAction(
        ...
        now,
        actionType,
        ActionTargetTypes.SOURCE_EXCERPT_PARAPHRASE,
        sourceExcerptParaphrase.id
      );
  ...
  switch (sourceExcerptType) {
    case SourceExcerptTypes.WRIT_QUOTE:
      return service.writQuotesService
        .readOrCreateWritQuoteAsUser(sourceExcerptEntity, userId, now)
        .then(({ isExtant, writQuote: sourceExcerptEntity }) => ({
          isExtant,
          sourceExcerptEntity,
        }));
    case SourceExcerptTypes.PIC_REGION:
      return service.picRegionsService
        .readOrCreatePicRegionAsUser(sourceExcerptEntity, userId, now)
        .then(({ isExtant, picRegion: sourceExcerptEntity }) => ({
          isExtant,
          sourceExcerptEntity,
        }));
    case SourceExcerptTypes.VID_SEGMENT:
      return service.vidSegmentsService
        .readOrCreateVidSegmentAsUser(sourceExcerptEntity, userId, now)
        .then(({ isExtant, vidSegment: sourceExcerptEntity }) => ({
          isExtant,
          sourceExcerptEntity,
        }));
    default:
