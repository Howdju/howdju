...
const { normalizeText, mapMany } = require("./daosUtil");
...
exports.PropositionsDao = class PropositionsDao {
  ...
  readPropositions(sorts, count) {
    ...
    const sql = `
      ...
      `;
    return this.database
      .query("readPropositions", sql, args)
  ...
  readMorePropositions(sortContinuations, count) {
    ...
    const sql = `
      ...
      `;
    return this.database
      .query("readMorePropositions", sql, args)
      .then(({ rows }) => map(rows, toProposition));
  }
  ...
  countEquivalentPropositions(proposition) {
    const sql = `
      ...
      `;
    return this.database
      .query("countEquivalentPropositions", sql, [
        normalizeText(proposition.text),
        proposition.id,
      ])
      .then((result) => {
  ...
  hasOtherUsersRootedJustifications(proposition, userId) {
    ...
    `;
    return this.database
      .query("hasOtherUsersRootedJustifications", sql, [
  ...
  hasOtherUsersRootedJustificationsVotes(proposition, userId) {
    ...
    `;
    return this.database
      .query("hasOtherUsersRootedJustificationsVotes", sql, [
        JustificationRootTargetTypes.PROPOSITION,
        proposition.id,
        userId,
      ])
