...
exports.PropositionCompoundsDao = class PropositionCompoundsDao {
  ...
  readPropositionCompoundEquivalentTo(propositionCompound) {
    ...
    const args = concat(
      ...
      flatMap(propositionCompound.atoms, (atom, index) => [index, normalizeText(atom.entity.text)])
    )
    const sql = `
      with
    ...
    return this.database.query(sql, args)
      .then( ({rows}) => {
        ...
        if (rows.length > 1) {
          this.logger.error(`${rows.length} equivalent proposition compounds`, {propositionCompound})
        }
  ...
  readPropositionAtomsByPropositionCompoundIdForRootPropositionId(rootPropositionId) {
    ...
    return this.database.query(sql, [rootPropositionId, JustificationBasisType.PROPOSITION_COMPOUND])
      .then( ({rows}) => {
        ...
        forEach(rows, row => {
          const propositionCompoundId = row.proposition_compound_id
          let propositionAtoms = propositionAtomsByPropositionCompoundId[propositionCompoundId]
          if (!propositionAtoms) {
            propositionAtomsByPropositionCompoundId[propositionCompoundId] = propositionAtoms = []
          }
