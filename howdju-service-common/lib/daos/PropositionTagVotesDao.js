...
const { toPropositionTagVote } = require("./orm");
...
exports.PropositionTagVotesDao = class PropositionTagVotesDao {
  ...
  readPropositionTagVote(userId, propositionId, tagId) {
    return this.database
      ...
      .then(
        mapSingle(this.logger, toPropositionTagVote, "proposition_tag_votes", {
          userId,
          propositionId,
          tagId,
        })
  ...
  deletePropositionTagVote(userId, propositionTagVoteId, now) {
    return this.database
      ...
      .then(
        ({ rows: [{ proposition_tag_vote_id }] }) => proposition_tag_vote_id
      );
  }
};
