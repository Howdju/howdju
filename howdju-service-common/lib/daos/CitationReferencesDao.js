...
exports.CitationReferencesDao = class CitationReferencesDao {
  ...
  readCitationReferencesEquivalentTo(citationReference) {
    ...
    return this.database.query(`
      ...
      .then( ({rows}) => {
        if (rows.length > 1) {
          this.logger.error(`${rows.length} equivalent citation references`, citationReference)
        }
  ...
  readCitationReferenceUrlsForCitationReference(citationReference) {
    return this.database.query('select * from citation_reference_urls where citation_reference_id = $1 and deleted is null',
      [citationReference.id])
      .then( ({rows}) => map(rows, toCitationReferenceUrl) )
  }

  readCitationReferenceUrl(citationReference, url) {
    return this.database.query('select * from citation_reference_urls where citation_reference_id = $1 and url_id = $2 and deleted is null',
      [citationReference.id, url.id])
      .then( ({rows}) => {
        if (rows.length > 1) {
          this.logger.error(`URL ${url.id} is associated with citation reference ${citationReference.id} multiple times`)
        }
  ...
  createCitationReference(citationReference, userId, now) {
    ...
    const quote = citationReference.quote || null
    return this.database.query(sql, [quote, citationReference.citation.id, userId, now])
  ...
  hasCitationReferenceChanged(citationReference) {
    const sql = `
      ...
      `
    return this.database.query(sql, [citationReference.id, citationReference.quote])
  ...
  deleteCitationReferenceUrls(citationReference, urls, now) {
    const sql = `
      update citation_reference_urls set deleted = $1 
        ...
        returning *
    `
    return this.database.query(sql, [now, citationReference.id, map(urls, url => url.id)])
      .then( ({rows}) => map(rows, toCitationReferenceUrl) )
  }
}
