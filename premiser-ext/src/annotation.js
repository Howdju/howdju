import {getNodeData, setNodeData} from './node-data'
...
/*
 ...
 - compaction
  ...
  // Determine annotations that are affected; increment all their ranges by 1
  // They might push other annotations too.  And it might be possible to compact those that were pushed.
  // Example:
  //
  //  Before:
...
export function getEquivalentAnnotation(nodes) {
  ...
  const partiallyCoextensiveAnnotations = getAnnotationsHavingCoextensiveNodes(firstNode)
  ...
  for (let i = 1; i < nodes.length; i++) {
    const node = nodes[i]
    const annotationsCoextensiveWithNode = getAnnotationsHavingCoextensiveNodes(node)
    const newAnnotationsByIndex = {}
    for (const encompassingAnnotation of annotationsCoextensiveWithNode) {
      if (partiallyCoextensiveAnnotationsByIndex[encompassingAnnotation.index]) {
        newAnnotationsByIndex[encompassingAnnotation.index] = encompassingAnnotation
      }
    }
  ...
  const potentialEquivalentAnnotations = objectValues(partiallyCoextensiveAnnotationsByIndex)
  ...
  if (equivalentAnnotations.length > 1) {
    logger.error(`multiple equivalent annotations`, nodes, partiallyCoextensiveAnnotationsByIndex)
  }
...
export function getNodesFor(startNode, startOffset, endNode, endOffset) {
  ...
  // First find the earliest descendant that doesn't contain endNode
  // do {
  //   curr = curr.firstChild
  // } while (curr && curr.firstChild.contains(endNode))
  // // then descend the tree grabbing all nodes that proceed endNode
  // while (curr && curr !== endNode) {
  //   nodes.push(curr)
  //   if (curr.nextSibling === endNode) {
  //     break
  //   }
  //   else if (curr.nextSibling && !curr.nextSibling.contains(endNode)) {
  //     curr = curr.nextSibling
  //   } else {
  //     curr = curr.firstChild
  //   }
...
function createAnnotation(targetNodes) {
  ...
  const annotation = new Annotation(annotations.length, annotationNodes, annotationLevel)
...
export function createAndInsertAnnotationNode(firstTargetNode, targetNode) {
  ...
  if (isAnnotationNode(targetNode)) {
  ...
  } else if ((ancestorAnnotationNodes = getAncestorAnnotationNodes(targetNode)).length) {
    let didInsert = false
    for (const ancestorAnnotationNode of ancestorAnnotationNodes) {
      ...
      if (!didInsert && nodeIsBefore(ancestorAnnotation.nodes[0], firstTargetNode)) {
    ...
    if (!didInsert) {
      const ancestorAnnotationNode = ancestorAnnotationNodes[ancestorAnnotationNodes.length-1]
...
function splitAnnotationNode(annotationNode, pivotNode) {
  ...
  let beforeClone = null, afterClone = null
  if (beforePivotChildren.length) {
    beforeClone = cloneWithChildren(annotationNode, beforePivotChildren, insertNodeBefore)
    annotation.insertNodeBefore(beforeClone, annotationNode)
  }
  if (afterPivotChildren.length) {
    afterClone = cloneWithChildren(annotationNode, afterPivotChildren, insertNodeAfter)
...
function getAncestorAnnotationNodes(node) {
  let curr = node, ancestorAnnotationNodes = []
  while (curr) {
    if (isAnnotationNode(curr)) {
...
function getAnnotationOf(annotationNode) {
  ...
  for (const node of annotation.nodes) {
    ...
    for (const intersectingAnnotation of annotationsIntersectingNode) {
      if (!seenAnnotationIndices[intersectingAnnotation.index]) {
        ...
        const recursiveAnnotations = findIntersectingAnnotations(intersectingAnnotation, seenAnnotationIndices)
...
function getOrCreateAnnotationLevelStyleElement() {
  let annotationLevelStyleElement = document.getElementById(annotationLevelStyleElementId)
  if (!annotationLevelStyleElement) {
    ...
    document.getElementsByTagName("head")[0].appendChild(annotationLevelStyleElement)
  }
...
function arrayInsertAfter(items, afterItem, item) {
  items.splice(items.indexOf(afterItem)+1, 0, item)
}

function arrayInsertBefore(items, afterItem, item) {
