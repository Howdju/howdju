...
describe('annotateNodes/getNodesFor', () => {
  test('annotates text', () => {
    ...
    const nodes = getNodesFor(textNode, textOffset, textNode, textOffset + text.length)
    annotateNodes(nodes)
  ...
  test('annotates across an element boundary', () => {
    ...
    const spanTextOffset = spanTextNode.textContent.indexOf(spanText) + spanText.length

    const nodes = getNodesFor(textNode, textOffset, spanTextNode, spanTextOffset)
    annotateNodes(nodes)
  ...
  test('supports overlapping annotation at the beginning', () => {
    document.body.innerHTML = `
      <p id="wrapper">
        Part of ${open(0, 1)}this will be annotated${close()} part of this will too
      </p>
    `.trim()
    const wrapper = document.getElementById("wrapper")
    const beforeTextNode = wrapper.childNodes[0]
    const annotationNode = wrapper.childNodes[1]
    const annotationTextNode = annotationNode.childNodes[0]
    const overlappingText = 'this will be'
    createAnnotation(annotationNode, 1)

    const nodes = getNodesFor(beforeTextNode, 0, annotationTextNode, overlappingText.length)
    annotateNodes(nodes)

    const expected = createElementFromHTML(`
      <p id="wrapper">${open(1, 1)}
        Part of ${close()}${open(1, 1)}${open(0, 2)}this will be${close()}${close()}${open(0, 2)} annotated${close()} part of this will too
      </p>
    `.trim())
    expect(wrapper).toBeEqualNode(expected)
  })
  ...
  test('supports overlapping annotation at the end', () => {
    ...
    const nodes = getNodesFor(annotationTextNode, startOffset, textNode, endOffset)
    annotateNodes(nodes)

    const expected = createElementFromHTML(`
      <p id="wrapper">
        Part of ${open(0, 1)}this will be ${close()}${open(0, 1)}${open(1, 2)}annotated${close()}${close()}${open(1, 2)} part of this${close()} will too
      </p>
    `.trim())
    expect(wrapper).toBeEqualNode(expected)
  })
  ...
  test('wraps an annotation', () => {
    ...
    const nodes = getNodesFor(beforeTextNode, 0, afterTextNode, afterTextNode.textContent.length)
    annotateNodes(nodes)

    const expected = createElementFromHTML(`
      <p id="wrapper">${open(1, 1)}
        All of this will be annotated at level 1 
        ${close()}${open(1, 1)}${open(0, 2)}this is 2${close()}${close()}${open(1, 1)} 
        this is back to 1
      ${close()}</p>
    `.trim())
    expect(wrapper).toBeEqualNode(expected)
  ...
  test('wraps an annotation again', () => {
    ...
    createAnnotation([wrapper.childNodes[1], wrapper.childNodes[2], wrapper.childNodes[3]], 1)
    createAnnotation(wrapper.childNodes[2].childNodes[0], 2)
    ...
    const expected = createElementFromHTML(`
      <p id="wrapper">${open(2, 1)}
        ...
        ${close()}${close()}${close()}${open(2, 1)}${open(0, 2)}this is back to 2
  ...
  test('annotates inside an annotation', () => {
    ...
    `.trim()
    const wrapper = document.getElementById("wrapper")
    const annotationNode = wrapper.childNodes[0]
    ...
    const expected = createElementFromHTML(`
      <p id="wrapper">${open(0, 1)}
        All of this will be annotated at level 1 
        ${close()}${open(0, 1)}${open(1, 2)}this is 2${close()}${close()}${open(0, 1)} 
        this is back to 1
      ${close()}</p>
    `.trim())
    expect(wrapper).toBeEqualNode(expected)
  ...
  test('annotates between two annotations', () => {
    ...
    `.trim()
    const wrapper = document.getElementById("wrapper")
    const annotation0Node0 = wrapper.childNodes[0]
    const annotation0Node1 = wrapper.childNodes[1]
    const annotation0Node2 = wrapper.childNodes[2]
    const annotation1Node = annotation0Node1.childNodes[0]
    ...
    const nodes = getNodesFor(textNode1, startOffset, textNode2, textNode2.nodeValue.indexOf('this is back'))
    annotateNodes(nodes)

    const expected = createElementFromHTML(`
      <p id="wrapper">${open(0, 1)}
        All of this will be annotated at level 1
        ${close()}${open(0, 1)}${open(2, 2)}Then a new annotation will annotate here 
        ${close()}${close()}${open(0, 1)}${open(2, 2)}${open(1, 3)}This will be all three
...
describe('getEquivalentAnnotation', () => {
  ...
  test('works', () => {
    document.body.innerHTML = `${open(0, 1)}The world is everything that is the case.${close()}`
    const annotationNode = document.body.childNodes[0]
    const textNode = annotationNode.childNodes[0]
    createAnnotation(annotationNode, 1)
...
function createAnnotation(annotationNodes, level) {
  ...
  setNodeData(annotationNodes, annotationIndexDataKey, annotation.index)
  return annotation
}
