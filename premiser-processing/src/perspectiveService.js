...
class PerspectivesDao {
  ...
  readFeaturedPerspectivesWithVotesForOptionalUserId(userId) {
    ...
    return this.database
      .query(
        "readFeaturedPerspectivesWithVotesForOptionalUserId",
        perspectiveJustificationsSql,
        args
      )
      .then(({ rows }) => {
        const transitiveRowsPromise =
          this._readFeaturedPerspectiveJustificationTransitiveJustifications(
            userId,
            1,
            4,
            rows
        ...
        if (rows.length > 0) {
          ...
          return (
            Promise.all([
              ...
              this._readFeaturedPerspectivesCounteredJustifications(
                userId,
                1,
                rows
              ),
              ...
              // TODO(1,2,3): remove exception
              // eslint-disable-next-line promise/no-nesting
              .then(([transitiveRows, counteredRows]) =>
                concat(transitiveRows, counteredRows)
              )
          );
        }
   ...
   */
  _readFeaturedPerspectiveJustificationTransitiveJustifications(
    userId,
    targetDepth,
    maxDepth,
    rows
    ...
    const votesJoinSql = userId
      ? `
        ...
        `
      : "";
    const transitiveSql = map(
      range(1, targetDepth + 1),
      (currentDepth) => `
    ...
    return this.database
      ...
      .then(({ rows: newRows }) => {
        ...
        return this._readFeaturedPerspectiveJustificationTransitiveJustifications(
          userId,
          targetDepth + 1,
          maxDepth,
          combinedRows
  ...
  _readFeaturedPerspectivesCounteredJustifications(userId, targetHeight, rows) {
    ...
    const votesJoinSql = userId
      ? `
        ...
        `
      : "";
    const justificationsJoinSqls = map(
      range(1, targetHeight + 1),
      (currentHeight) => `join justifications j${currentHeight} on
                    j${currentHeight - 1}.target_type = $2
                and j${
                  currentHeight - 1
                }.target_id = j${currentHeight}.justification_id
    ...
    return this.database
      ...
      .then(({ rows: newRows }) => {
        if (newRows.length > 0) {
          return this._readFeaturedPerspectivesCounteredJustifications(
            userId,
            targetHeight + 1,
            rows.concat(newRows)
          );
  ...
  _addPropositionAndMapPerspectives(rows) {
    ...
    forEach(rows, (row) => {
      ...
      if (!perspective) {
        perspectivesById[row.perspective_id] = perspective = {
      ...
      if (!rootPropositionIdByPerspectiveId[perspective.id]) {
        rootPropositionIdByPerspectiveId[perspective.id] =
          row.root_proposition_id;
      }
    });

    return Promise.all(
      map(
        rootPropositionIdByPerspectiveId,
        (rootPropositionId, perspectiveId) =>
          Promise.all([
            this.propositionCompoundsDao.readPropositionCompoundsByIdForRootPropositionId(
              rootPropositionId
            ),
            this.writQuotesDao.readWritQuotesByIdForRootPropositionId(
              rootPropositionId
            ),
          ]).then(([propositionCompoundsById, writQuotesById]) => {
            const {
              rootJustifications,
            ...
            return Promise.props({
              perspectiveId,
              proposition:
                this.propositionsDao.readPropositionForId(rootPropositionId),
              justifications,
            });
          })
      )
      ...
      return map(
        rootPropositionAndPerspectiveIds,
        ({ perspectiveId, proposition, justifications }) => {
