...
const {
  ...
  logEventToDocument,
  indexDocuments,
} = require("./logTransforms");

const elasticsearchAuthority = process.env.ELASTICSEARCH_AUTHORITY;
if (!elasticsearchAuthority)
  throw new Error("ELASTICSEARCH_AUTHORITY is required");
...
module.exports.handler = async (event, context, callback) => {
  ...
  const documents = logEvents.map((logEvent) =>
    logEventToDocument(logEvent, logGroup, logStream)
  );
  const successCount = await indexDocuments(
    documents,
    client,
    elasticsearchIndex,
    elasticsearchType,
