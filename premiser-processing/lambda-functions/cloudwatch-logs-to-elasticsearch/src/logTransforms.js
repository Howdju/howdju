...
const unexpectedTokenRegExp = /SyntaxError: Unexpected token (.+) in JSON at position (\d+)/
...
module.exports.extract = async function extract(event) {
  ...
  const text = unzippedData.toString('ascii')
  ...
  try {
    ...
    if (match) {
      ...
      const offendingContext = text.substring(position - contextLines, position + contextLines)
      logger.error(`Invalid JSON (${token}) at ${contextLines}: ${offendingContext}`)
    } else {
...
const reportPhaseRegExp = /^REPORT RequestId: ([\w-]+)\s+Duration: ([\d.]+) ms\s+Billed Duration: ([\d.]+) ms\s+Memory Size: (\d+) MB\s+Max Memory Used: (\d+) MB\s*$/
/**
 ...
 */
module.exports.logEventToDocument = function logEventToDocument(logEvent, logGroupName, logStreamName) {
...
module.exports.indexDocuments = async function indexDocuments(parsedEvents, client, index, type, timeout) {
  // https://www.elastic.co/guide/en/elasticsearch/client/javascript-api/current/api-reference.html
  const bulkRecords = []
  for (const event of parsedEvents) {
    bulkRecords.push({ "index" : { _index : index, _type: type, _id : event.requestId } })
...
function extractResponseFailures(responseItems) {
  ...
  for (const responseItem of responseItems) {
    ...
    if (!shards) {
      logger.warn(`Elasticsearch bulk response action result lacks _shards property: ${JSON.stringify(actionResult)}`)
