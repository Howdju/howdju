...
describe("logTransforms", () => {
  ...
  describe("logEventToDocument", () => {
    test("creates document for start phase logEvent", () => {
      ...
      expect(logEventToDocument(logEvent, logGroupName, logStreamName)).toEqual(
    ...
    test("creates document for logger logEvent", () => {
        ...
        response: {
          ...
          headers: {
            ...
            "Access-Control-Allow-Headers":
              "Content-Type,Content-Encoding,Authorization,Request-ID,Session-Storage-ID,Page-Load-ID",
            ...
            propositions: [
              {
                ...
                normalText:
                  "nasa awarded spacex a 16b contract to supply the international space station at the end of 2008",
      ...
      const logEventMessage =
        `{"timestamp":"${timestamp}","context":{"clientRequestId":"${clientRequestId}",` +
        `"serverRequestId":"${serverRequestId}","stage":"${stage}"},"level":"${logLevel}",` +
        `"levelNumber":${logLevelNumber},"message":"${message}","data":${JSON.stringify(
          data
        )}}`;
