...
if (!fs.existsSync(lambdaDir))
  throw new Error(`lambda function directory does not exist: ${lambdaDir}`);
...
gulp.task("clean", (next) => del([lambdaBuildDir, lambdaDistDir], next));

gulp.task("copy-src", () => {
  const src = ["src/**/*.js", "!src/**/*.test.js"];
  ...
  return gulp.src(src, options).pipe(gulp.dest(lambdaBuildDir));
});

gulp.task("copy-package-json", () =>
  gulp.src("package.json", { cwd: lambdaDir }).pipe(gulp.dest(lambdaBuildDir))
);

gulp.task("install", () =>
  gulp
    .src("./package.json", { cwd: lambdaDir })
...
gulp.task("install-local", () => {
  const localInstaller = new LocalInstaller(
    {
      [lambdaBuildDir]: ["../howdju-common", "../howdju-service-common"],
    }
