...
cmp --silent dist/rangeToFragment.js dist/tmp/rangeToFragment.js || {
  echo "rangeToFragment.js is not up to date. Please run \`yarn build:range-to-fragment-script\` and commit the changes."
  exit 1
}
...
cmp --silent dist/getRangesForCurrentFragment.js dist/tmp/getRangesForCurrentFragment.js || {
  echo "getRangesForCurrentFragment.js is not up to date. Please run \`yarn build:get-ranges-for-current-fragment-script\` and commit the changes."
  exit 1
}
