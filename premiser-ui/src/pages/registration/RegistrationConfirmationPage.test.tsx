...
describe('RegistrationConfirmationPage', () => {
  test('shows form', async () => {
    ...
    // Act
    const {container, history} = renderWithProviders(<RegistrationConfirmationPage />)
    history.push(`complete-registration?registrationCode=${registrationCode}`)
    await screen.findByLabelText(/username/i)

    // Assert
    expect(screen.getByRole('heading')).toHaveTextContent(/complete registration/i)
  ...
  test('shows error message for duplicate username', async () => {
    ...
    await user.click(screen.getByRole('checkbox', {name: /read and agree to the User Agreement/i}))
    await user.click(screen.getByRole('checkbox', {name: /13 years old or older/i}))
    await user.click(screen.getByRole('checkbox', {name: /consent to the processing of my personal data/i}))
    await user.click(screen.getByRole('checkbox', {name: /I am not located in the European Union/i}))
