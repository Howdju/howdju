...
export default function* handleTransientInteractions() {
  yield takeEvery(
    str(ui.unhandledAppClick),
    function* unhandledAppClickWorker() {
      yield put(ui.hideAllTransients());
    }
  );

  yield takeEvery(
    str(ui.hideAllTransients),
    function* hideAllTransientsWorker() {
      yield all(
        map(
          delayedHideTransientTaskByTransientId,
  ...
  yield takeEvery(
    str(ui.beginInteractionWithTransient),
    function* beginInteractionWithTransientWorker(action) {
  ...
  yield takeEvery(
    str(ui.endInteractionWithTransient),
    function* endInteractionWithTransientWorker(action) {
      const transientId = action.payload.transientId;
  ...
  yield takeEvery(
    str(ui.hideOtherTransients),
    function* hideOtherTransientsWorker(action) {
      const visibleTransientId = action.payload.visibleTransientId;

      yield all(
        map(delayedHideTransientTaskByTransientId, (task, transientId) =>
  ...
  yield takeEvery(
    str(ui.scheduleDelayedHideTransient),
    function* scheduleDelayedHideTransientWorker(action) {
      const { transientId, hideDelay } = action.payload;

      yield put(ui.tryCancelDelayedHideTransient(transientId, action));
      delayedHideTransientTaskByTransientId[transientId] = yield fork(
        delayedHide,
        hideDelay,
        transientId,
        action
  ...
  yield takeEvery(
    str(ui.tryCancelDelayedHideTransient),
    function* cancelDelayedHideTransientWorker(action) {
      const transientId = action.payload.transientId;
  ...
  yield takeEvery(
    str(ui.cancelDelayedHideTransient),
    function* cancelDelayedHideTransientWorker(action) {
      const transientId = action.payload.transientId;
