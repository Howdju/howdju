...
export function* postExtensionMessages() {
  ...
  yield takeEvery(extensionActionCreators, function* postExtensionMessagesWorker(action) {
    if (!inIframe()) {
      throw new Error("should not call extension action when we are not in an extension iframe.")
    }
    logger.trace(`difficult postExtensionMessagesWorker ${JSON.stringify({action})}`)
...
export function* contentScriptAck() {
  yield takeEvery(actions.extensionFrame.ackMessage, function* ackMessageWorker(action) {
