...
import { apiErrorCodes, schemaIds, schemaSettings } from "howdju-common";
import { validate } from "howdju-ajv-sourced";

import { api, pages, mapActionCreatorGroupToDispatchToProps } from "./actions";
import PasswordTextField from "./PasswordTextField";
import paths from "./paths";
import { selectPasswordResetConfirmationPage } from "./selectors";
...
class PasswordResetConfirmationPage extends React.Component {
  ...
  render() {
    ...
    const { dirty, wasSubmitAttempted } = this.state;

    const { isValid, errors: validationErrors } = validate(
      schemaIds.passwordResetConfirmation,
      passwordResetConfirmation
    );

    const errorNoticeMessage =
        ...
        <FocusContainer focusOnMount containFocus={false}>
          <CardText>
            <PasswordTextField
              ...
              required
              error={
                (dirty.newPassword || wasSubmitAttempted) &&
    ...
    const formWithNotice = (
      <React.Fragment>
        ...
        {errorNoticeMessage && (
          <CardText className="error-message">{errorNoticeMessage}</CardText>
        )}
        {form}
      </React.Fragment>
    );
  ...
  onSubmit = (event) => {
    ...
    this.props.api.confirmPasswordReset(
      this.props.passwordResetCode,
      this.props.passwordResetConfirmation
    );
  };
