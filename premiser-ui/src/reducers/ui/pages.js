...
import { EntityTypes, httpStatusCodes } from "howdju-common";

import { api, pages } from "../../actions";
    ...
    [api.requestPasswordReset.response]: {
      ...
      throw: (state, action) => {
        ...
        if (
          ...
          errors.email = {
            value: get(action, "payload.body.identifier.email"),
          };
        }
...
export const passwordResetConfirmationPage = handleActions(
  {
    ...
    [combineActions(
      ...
      throw: (state, action) => ({
        ...state,
        didCheck: true,
        errorCode: get(action, "payload.body.errorCode"),
      }),
    },
  },
  passwordResetConfirmationPageDefaultState
