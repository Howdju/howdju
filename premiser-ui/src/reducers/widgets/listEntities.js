...
const widgetResponseReducer =
  (defaultWidgetState, entitiesWidgetStateKey, entitiesResultKey) =>
  (state, action) => {
    ...
    const newWidgetState = {
      [entitiesWidgetStateKey]: union(
        widgetState[entitiesWidgetStateKey],
...
const widgetDeleteResponseReducer =
  ...
  (state, action) => {
    ...
    const widgetState = get(state, widgetId, defaultWidgetState);
    const newWidgetState = {
      [entitiesWidgetStateKey]: pickBy(
        widgetState[entitiesWidgetStateKey],
        (e, id) => id !== action.payload.result[entitiesResultKey].id
      ),
    };
...
export default handleActions(
  {
    [api.fetchRecentPropositions]: widgetRequestReducer(
    ...
    [api.fetchRecentWritQuotes]: widgetRequestReducer(
    ...
    [api.fetchRecentJustifications]: widgetRequestReducer(
