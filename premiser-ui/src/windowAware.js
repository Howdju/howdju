...
export default function windowAware(WrappedComponent) {
  class WindowAware extends React.Component {
    render() {
      const { isWindowNarrow, ...rest } = this.props;
      return <WrappedComponent {...rest} isWindowNarrow={isWindowNarrow} />;
    }
  }
  WindowAware.displayName = `WindowAware(${getComponentDisplayName(
    WrappedComponent
  )})`;
