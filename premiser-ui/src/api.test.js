...
import { statementSchema, statementsSchema, statementJustificationsSchema} from './schemas'
...
describe('api', () => {
  test('fetchStatements', done => {
    ...
    Promise.all([import('isomorphic-fetch'), import('./api')]).then( ([fetch, api]) => {
      ...
      fetch.mockImplementation( () => Promise.resolve({ ok: true, json: () => Promise.resolve(statements) }) )
