...
const slice = createSlice({
  name: "MediaExcerptApparitionsDialog",
  initialState,
  reducers: {
...
export const selectIsDialogVisible = (state: RootState) =>
  state.mediaExcerptApparitionsDialog.isDialogVisible;

export function* mediaExcerptApparitionsDialogSaga() {
  yield takeEvery(
    slice.actions.showDialog,
    // TODO(525) factor out the duplicate logic in these sagas. In particular, this slice is nearly
    // identical to propositionAppearancesDialogSlice.
    function* ({ payload: mediaExcerptId }) {
      const {
        payload: {
          fetchInit: { requestId },
        },
      } = yield put(api.fetchMediaExcerptApparitions(mediaExcerptId));
      const {
        responseAction,
        timeout,
      }: {
        responseAction: ReturnType<
          typeof api.fetchMediaExcerptApparitions.response
        >;
        timeout: true;
      } = yield race({
        responseAction: call(
          getResponseAction,
          requestId,
          api.fetchMediaExcerptApparitions.response
        ),
