...
const { gitCommitMetadataKey } = require("howdju-common");
const { gitSha, logger } = require("howdju-ops");
...
upload.send(function (err, data) {
  ...
  progress.stop();
  logger.info(
    `Successfully uploaded ${zipPath} to ${data.Bucket}/${data.Key} (Etag: ${data.ETag})`
  );
});
